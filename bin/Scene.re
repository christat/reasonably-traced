...
let computeColor = (record: Record.t) =>
  Vec3f.addConst(1.0, record.normal)
  |> Vec3f.mulConst(0.5);

let computeBackgroundColor = (ray: Ray.t) => {
...
let rec intersection = (~ray: Ray.t, ~min_t: float=0.0, ~max_t: float=infinity, ~record: option(Record.t)=None, items: t) =>
  switch (items) {
  | [] =>
    switch record {
